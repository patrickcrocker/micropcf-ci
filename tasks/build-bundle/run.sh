...
forge_release_version=$(git -C forge-release describe)
vagrant_box_version=$(cat vagrant-box-version/number)
forge_tgz_url=$(cat forge-tgz/url)

output_dir=forge-bundle-$forge_release_version
...
sed "$box_version_filter" forge-release/vagrant/Vagrantfile >> $output_dir/vagrant/Vagrantfile

cp -r forge-release/terraform/aws $output_dir/terraform/
