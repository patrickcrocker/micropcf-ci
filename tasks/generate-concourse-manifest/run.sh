...
cp micropcf-ci/tasks/generate-concourse-manifest/manifest.yml .
...
sed -i "s%VSPHERE-WORKER-PUBLIC-KEY%$VSPHERE_WORKER_PUBLIC_KEY%g" manifest.yml
sed -i "s%TSA-PUBLIC-KEY%$CONCOURSE_TSA_PUBLIC_KEY%g" manifest.yml
sed -i "s%TSA-PRIVATE-KEY%\"$FORMATTED_CONCOURSE_TSA_PRIVATE_KEY\"%g" manifest.yml
