...
micropcf_json=$(cat vagrant-image-changes/vagrant/micropcf.json)
post_processor_json=`
cat <<EOF
{
  "post-processors": [[
...
echo $micropcf_json | jq '. + '"$post_processor_json" > vagrant-image-changes/vagrant/micropcf.json
